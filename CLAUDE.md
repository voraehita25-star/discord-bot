...
| --- | --- | --- | --- |
| Bot core | `bot.py`, `cogs/`, `utils/`, `config.py` | Python 3.14+ — discord.py, Anthropic Claude (`claude-opus-5`), Gemini, FAISS RAG, yt-dlp/spotipy | ~6,389 pytest |
...
- **Security posture is a feature.** The codebase hardens against SSRF (DNS-rebind + IPv6), path traversal (`safe_delete` confined to `temp/`), secret leakage (regex log redaction), and Discord mention abuse (sanitization + `AllowedMentions`). Preserve these when editing `utils/web/`, `utils/reliability/`, and `cogs/ai_core/sanitization.py`. The memory screen carries TWO denylists: the generic LLM-injection one, and `_PROMPT_STRUCTURE_MARKERS` — this repo's own prompt boundaries (`---END SYSTEM CONTEXT---`, `[System Info]`, the flattener's `#` headings). A stored fact is replayed INSIDE those boundaries on every future turn, so one carrying them can close the system-context section early or forge a `| User:` header; that only became reachable once `!remember` was wired into the prompt. Pickle/`.npy` RAG loading is gated behind `RAG_ALLOW_LEGACY_PICKLE` (off by default) — keep it off. The dashboard CLI backend's autonomous file-write mode is similarly gated behind `DASHBOARD_CLI_ALLOW_WRITE` (off by default): when on, the embedded `claude -p` may create/edit files non-interactively, but only inside `DASHBOARD_CLI_WRITE_DIRS` (default the user's Desktop/Documents/Downloads). It is files-only (Bash/web/NotebookEdit/Task denied) and confined by the `cogs/ai_core/api/cli_write_guard.py` PreToolUse hook (fail-closed, exit 2) — preserve that hook as the authoritative path boundary.
...
- **The token math was calibrated for a language this bot does not speak.** Measured with cl100k_base against the bot's own stored history (267,614 chars of Thai RP): non-ASCII is **1.046** chars/token and the corpus overall **1.116** — not the "~2-3" the fallback estimator assumed nor the "~1-2" the prompt ceiling was sized on. Two consequences, both fixed: `_estimate_tokens_fallback` under-counted Thai by 58%, and it drives `smart_trim_by_tokens` — the over-limit "ย่อประวัติ" button — so an over-window history looked like it already fit and the trim removed nothing while reporting success; and `CLI_PROMPT_MAX_CHARS`'s 1,200,000 default implied ~1,075,000 tokens against a 1M window, so the ceiling whose purpose is preventing context overflow was permitting it (now 900,000 ≈ 806K tokens). If you re-tune either, re-measure — don't reason from English rules of thumb.
