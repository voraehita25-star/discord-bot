...
try {
    ...
    if (Test-Path $sourceExe) {
        ...
        # Copy to release folder with legacy English name
        $englishExeRelease = Join-Path $releaseDir "Discord Bot Dashboard.exe"
        Copy-Item $sourceExe $englishExeRelease -Force
        Write-Host "  -> $releaseDir\Discord Bot Dashboard.exe" -ForegroundColor Green
        ...
        # Copy to BOT folder (legacy English name)
        $legacyEnglishExeBot = Join-Path $botDir "Discord Bot Dashboard.exe"
        Copy-Item $sourceExe $legacyEnglishExeBot -Force
        Write-Host "  -> BOT\Discord Bot Dashboard.exe" -ForegroundColor Green
