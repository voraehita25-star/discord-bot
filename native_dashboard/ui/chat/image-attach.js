...
export class ImageAttachManager {
     ...
     */
    setup(documents, isStreaming) {
        const streamingNow = () => (isStreaming ? isStreaming() : false);
        ...
        fileInput?.addEventListener('change', () => {
            ...
            if (files) {
                ...
                const remaining = Math.max(0, MAX_ATTACHED_IMAGES - (this.images.length + this.pendingCount));
                imageFiles.slice(0, remaining).forEach(file => this.attach(file));
                if (imageFiles.length > remaining) {
                    showToast(`Only ${remaining} more image${remaining === 1 ? '' : 's'} could be `
                        + `attached (max ${MAX_ATTACHED_IMAGES}).`, { type: 'warning' });
                }
                picked.forEach(file => {
