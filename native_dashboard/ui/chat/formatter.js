...
function applyLinks(html) {
    ...
    const withMdLinks = html.replace(MD_LINK_RE, (_match, text, url) => {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (i % 2 === 1)
            continue;
        segments[i] = segments[i].replace(BARE_URL_RE, (_m, pre, url) => {
            const [cleanUrl, trail] = splitTrailingPunctuation(url);
...
let linkHookInstalled = null;
function ensureLinkHook(purify) {
    if (linkHookInstalled === purify)
        return;
 ...
 */
const BLOCK_OPEN = '(?:<(?:ul|ol|blockquote|pre|table|hr|h[1-6])\\b' +
 ...
 */
function tightenBlockSpacing(html) {
...
function formatMessageUncached(content) {
    ...
    html = html.replace(/(^|[^A-Za-z0-9_])__(?!\s)([^_\n]+?)(?<!\s)__(?![A-Za-z0-9_])/g, '$1<strong>$2</strong>');
    ...
    html = html.replace(/^(?![ \t]*$)(?![#<|]|&gt;|[-*+][ \t]|\d{1,9}[.)][ \t])([^\n]+)\n=+[ \t]*$/gm, '<h1 class="md-heading">$1</h1>');
    ...
    for (const level of [6, 5, 4, 3, 2, 1]) {
        html = html.replace(new RegExp(`^#{${level}}${ATX_TAIL}`, 'gm'), `<h${level} class="md-heading">$1</h${level}>`);
    }
