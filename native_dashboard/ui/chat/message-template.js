...
export function computeWindow(total, visibleMessageCount) {
    const shouldVirtualize = total > VIRT_THRESHOLD;
    let requested = visibleMessageCount;
    if (shouldVirtualize && requested <= 0)
        requested = VIRT_WINDOW_SIZE;
...
export function renderMessagesHtml(ctx) {
...
function renderSingleMessage(msg, msgIdx, mctx) {
