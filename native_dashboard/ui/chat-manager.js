 ...
 */
import { invoke, errorLogger, escapeHtml, isSafeAvatarUrl, safeAvatarUrl, showToast, showConfirmDialog, settings, saveSettings, icon, normalizeSqliteUtc, refreshSendButtonGlow, countLabel, scrollBehavior, } from './shared.js';
import { highlightCodeBlocks } from './chat/prism.js';
import { formatMessage, formatStreamingMessage, stripThinkTags, stripThinkTagsStreaming, } from './chat/formatter.js';
...
export class ChatManager {
    ...
    handleMessage(data) {
        switch (data.type) {
            ...
            case 'conversation_loaded': {
                ...
                {
                    const convProvider = this.currentConversation.ai_provider;
                    if (convProvider &&
            ...
            case 'document_saved':
                ...
                // across restarts (scoped to this conversation only).
                {
                    ...
                    const pendingChars = docs.reduce((s, d) => s + (typeof d.char_count === 'number' ? d.char_count : 0), 0);
                    this.contextWindow.addPendingDocumentChars(data.conversation_id
                        ?? this.currentConversation?.id ?? null, pendingChars);
                }
                break;
            case 'conversation_documents': {
                ...
                this.contextWindow.setDocumentFootprint(convId ?? null, totalChars, cw, !!convId && this.currentConversation?.id === convId);
                break;
            }
    ...
    sendMessage() {
        ...
        const writeToggle = document.getElementById('chat-write-mode');
    ...
    retryFailedSend(idx) {
        ...
        if (!msg || msg.role !== 'user' || !msg.failed)
            return;
        if (this.isStreaming) {
            showToast('Wait for the current response to finish', { type: 'warning' });
            return;
    ...
    restoreStreamingBubble() {
        ...
        if (this.streamPartialChunks.length)
    ...
    appendChunk(text) {
        ...
        if (!document.querySelector('#streaming-message .streaming-text'))
            return;
    ...
    renderStreamingPartial() {
        ...
        // would be the bug.
        streamingText.innerHTML = formatStreamingMessage(stripThinkTagsStreaming(this.streamPartialChunks.join('')));
     ...
     */
    updateActionButton(opts) {
        const btn = document.querySelector(`.${opts.btnClass}[data-msg-id="${opts.id}"]`);
        if (!btn)
            return false;
    ...
    toggleThinking(header) {
        ...
        const id = header.closest('.thinking-container')?.dataset.thinkingId;
        if (id) {
            if (collapsed)
                this.expandedThinking.delete(id);
            else
                this.expandedThinking.add(id);
        }
    }
    ...
    renderMessages() {
        ...
        if (this.messages.length > this.lastRenderedCount) {
            container.lastElementChild?.classList.add('is-new');
        }
        ...
        container.querySelectorAll('.thinking-header.collapsible').forEach(header => {
            this.bindThinkingToggle(header);
        });
    ...
    setInputEnabled(enabled) {
        ...
        if (enabled && this.composerHadFocus) {
            ...
            if (idle && onChatPage && input && !input.disabled)
                input.focus();
        }
    }
    ...
    static SCROLL_FAB_BASE_LABEL = 'Scroll to bottom';
    ...
    updateScrollFab(show) {
        ...
        if (n > 0) {
            fab.setAttribute('aria-label', `Scroll to latest, ${n} new message${n === 1 ? '' : 's'}`);
        }
        else {
        ...
        list.querySelectorAll('.file-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const name = el.closest('.chat-file-row')?.querySelector('.file-name')?.textContent
                    || 'this file';
                const ok = await showConfirmDialog(`Remove "${name}" from this conversation's memory? Its extracted text is deleted and the AI will stop seeing it.`);
                // Re-check the conversation: it can change while the dialog is up.
                if (!ok || !this.currentConversation)
                    return;
                this.send({
     ...
     */
    applyToggleSupport(input, label, defaultTitle, info) {
    ...
    applyToggleSupportFromHandshake(data) {
        const thinking = data.thinking_toggle;
        const thinkingOk = this.applyToggleSupport(document.getElementById('thinking-toggle'), document.querySelector('.thinking-toggle'), 'Enable Thinking Mode', thinking);
        this.applyToggleSupport(document.getElementById('modal-thinking'), document.getElementById('modal-thinking')?.closest('.option-row'), '', thinking);
        if (!thinkingOk)
            this.thinkingEnabled = false;
        this.applyToggleSupport(document.getElementById('chat-use-search'), document.getElementById('chat-use-search-label'), 'Let the assistant search the web', data.web_search_toggle);
        this.applyToggleSupport(document.getElementById('chat-write-mode'), document.getElementById('chat-write-mode-label'), 'Let the assistant create and edit files', data.write_mode_toggle);
    }
    /**
     ...
     */
    resolveSearchWriteConflict(searchOn, writeOn) {
        if (!(searchOn && writeOn))
            return writeOn;
        const writeToggle = document.getElementById('chat-write-mode');
        if (writeToggle)
            writeToggle.checked = false;
        showToast('Write mode turned off — the assistant cannot search the web and write files in the same message.', { type: 'warning' });
        return false;
    }
    ...
    downloadExport(data) {
        ...
        if (bulkItem) {
            ...
            if (this.bulkExportRemaining === 0) {
                showToast(`Exported ${countLabel(this.bulkExportTotal, 'conversation')} to your downloads.`, { type: 'info' });
            }
        }
        else {
        ...
        document.getElementById('btn-export-all')?.addEventListener('click', async () => {
            ...
            window.setTimeout(() => {
                if (this.bulkExportRemaining > 0) {
                    ...
                    showToast(`Exported ${done} of ${countLabel(this.bulkExportTotal, 'conversation')}.`, { type: 'warning' });
