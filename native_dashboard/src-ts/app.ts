 ...
 * actually used.
 */
async function revealSettingsSection(card: HTMLElement): Promise<void> {
    ...
    if (page && typeof page.getAnimations === 'function') {
        ...
        await Promise.all(
            page.getAnimations().map((a) => a.finished.catch(() => undefined)),
        );
    }
...
function buildSettingsSectionCommands(): Command[] {
    const cards = document.querySelectorAll<HTMLElement>('#page-settings .settings-card');
    return Array.from(cards).flatMap((card, i): Command[] => {
        ...
        const captions = Array.from(
            card.querySelectorAll<HTMLElement>('.setting-row label, .setting-row .setting-label'),
        ).map((l) => l.textContent ?? '');
...
function applyChatRailCollapsed(collapsed: boolean): void {
...
export function initChatRailToggle(): void {
...
let sakuraRenderer: SakuraRenderer | null = null;
...
export function sakuraDebugState(): { running: boolean; count: number; frames: number } {
...
export function setSakuraEnabled(enabled: boolean): void {
    ...
    if (enabled) {
        if (!sakuraRunning) initSakuraAnimation();
    } else {
        stopSakura();
    }
}
...
function initSakuraAnimation(): void {
    ...
    const NIGHT_COLORS: Array<[number, number, number]> = [
        [1.000, 0.816, 0.886],
        [1.000, 0.780, 0.860],
        [1.000, 0.706, 0.812],
        [1.000, 0.643, 0.769],
    ...
    const DAWN_COLORS: Array<[number, number, number]> = [
        [0.937, 0.561, 0.694],
        [0.898, 0.455, 0.624],
        [0.949, 0.651, 0.757],
        [0.851, 0.416, 0.580],
    ...
    // 31 petals to 37.
    const MAX_PETALS = Math.max(16, Math.min(44, Math.round(
     ...
     *
     *  The model is a thin flat plate falling through still-ish air. Everything
     *  the eye reads as "petal motion" — the zigzag, the hesitation, the sudden
     *  slip, the flash as it turns edge-on — comes out of ONE quantity: how
     *  side-on the plate currently is to the airflow (`tumble`). Broadside means
     *  maximum drag (slow fall) and maximum lift (hard sideways swing); edge-on
     *  means almost no drag (it drops) and no lift (it goes straight). The old
     *  model painted that shape by hand with three tuned sines instead, which is
     *  why it read as a loop rather than as falling. */
    interface PetalPhysics {
        x: number; y: number;       // px, container space
        vx: number; vy: number;     // px/s
        angle: number;              // deg — in-plane rotation
        size: number;               // px
        life: number;               // s since spawn (drives the fade-in)
        depth: number;              // 0 far … 1 near — drives size/speed/opacity
        tumble: number;             // rad — rotation about the petal's own axis
     ...
     */
    function setBaseTint(p: PetalPhysics): void {
     ...
     */
    function resetPetal(p: PetalPhysics, width: number, height: number, seeded: boolean): void {
    ...
    const LIGHT_DIR: [number, number, number] = [-0.46, -0.56, 0.69];
    ...
    syncRendererSize();
    const resizeHandler = (): void => syncRendererSize();
    ...
    for (let i = 0; i < MAX_PETALS; i++) createPetal(true);
 ...
 */
function restorePaneScroll(page: string): void {
    const id = PANE_SCROLL_IDS[page];
    if (id === undefined) return;
...
export function _resetPaneScrollState(): void {
 ...
 */
function statPlaceholder(value: string | null | undefined): string {
    const v = (value ?? '').trim();
    return v === '' || v === '-' ? '—' : v;
}
