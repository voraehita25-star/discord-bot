...
/** One row in the palette. `run` is the same callback the real control uses. */
export interface Command {
    id: string;
    label: string;
    /** Section heading, used only when the query is empty. */
    group: string;
    /** Sprite id WITHOUT the `#i-` prefix — must be one of the 58 real glyphs. */
    icon: string;
    /** Extra search terms that are not in the label ("dark", "light", "petals"). */
    keywords?: string;
    /** Existing keyboard chord, shown right-aligned. */
    hint?: string;
    run: () => void;
}

/** A command that matched, plus where it matched (for highlighting). */
export interface CommandMatch {
    command: Command;
    /** Indices into `command.label` that the query hit. Empty for keyword-only hits. */
    positions: number[];
    score: number;
}
...
export function scoreCommand(command: Command, query: string): CommandMatch | null {
    ...
    if (!q) return { command, positions: [], score: 0 };
    ...
    for (let i = 0; i < label.length && qi < q.length; i++) {
        if (label[i] !== q[qi]) continue;
        positions.push(i);
        // Word start: index 0, or preceded by a space/dash/slash.
        const prev = i > 0 ? label[i - 1] : ' ';
        if (i === 0 || prev === ' ' || prev === '-' || prev === '/') score += 12;
        if (i === lastHit + 1) score += 8;
        lastHit = i;
        qi++;
    }
...
export function filterCommands(commands: readonly Command[], query: string): CommandMatch[] {
    const matches: Array<CommandMatch & { order: number }> = [];
    commands.forEach((command, order) => {
        const m = scoreCommand(command, query);
        if (m) matches.push({ ...m, order });
    });
 ...
 */
export function buildLabel(label: string, positions: readonly number[]): DocumentFragment {
...
/** The DOM the palette drives. Passed in so the module can be unit-tested. */
...
/** Hooks into app.ts's modal contract, so inert/focus behave like every modal. */
...
export class CommandPalette {
    private readonly el: PaletteElements;
     ...
     */
    private readonly provide: () => readonly Command[];
    private commands: readonly Command[] = [];
    private matches: CommandMatch[] = [];
    private activeIndex = 0;

    constructor(el: PaletteElements, host: PaletteHost, provide: () => readonly Command[]) {
        this.el = el;
        this.host = host;
    ...
    private bind(): void {
        ...
        this.el.input.addEventListener('keydown', (e) => {
            switch (e.key) {
                ...
                case 'ArrowUp':
                    ...
                    return;
                case 'Home':
                    if (this.matches.length) { e.preventDefault(); this.setActive(0); }
                    return;
                case 'End':
                    if (this.matches.length) { e.preventDefault(); this.setActive(this.matches.length - 1); }
                    return;
                case 'Enter':
                    e.preventDefault();
                    this.runActive();
        ...
        this.el.list.addEventListener('click', (e) => {
            const row = (e.target as HTMLElement).closest<HTMLElement>('.cmdk-item');
            if (!row) return;
            const idx = Number(row.dataset.index);
            if (Number.isInteger(idx)) { this.setActive(idx); this.runActive(); }
        });
        ...
        this.el.list.addEventListener('mousemove', (e) => {
            ...
            if (!row) return;
            const idx = Number(row.dataset.index);
            if (Number.isInteger(idx) && idx !== this.activeIndex) this.setActive(idx);
        });
    ...
    private setActive(index: number): void {
        ...
        const rows = this.el.list.querySelectorAll<HTMLElement>('.cmdk-item');
        rows.forEach((row, i) => {
            const on = i === index;
        ...
        if (!rows.length) this.el.input.removeAttribute('aria-activedescendant');
    }
    ...
    private render(query: string): void {
        ...
        let lastGroup = '';
        let container: HTMLElement = this.el.list;
    ...
    private buildRow(match: CommandMatch, index: number): HTMLElement {
