 ...
 */
const PERMANENT_HISTORY_ERROR_CODES: ReadonlySet<string> = new Set([
    'ROW_CONFLICT',
    'MSG_NOT_FOUND',
    'INVALID_ID',
    'INVALID_PAYLOAD',
...
export interface AiHistoryChannel {
    /** Discord snowflake as a string (exceeds Number.MAX_SAFE_INTEGER). */
    channel_id: string;
    /** "GuildName / #channel" or "Channel <id>" fallback. */
    name: string;
    message_count: number;
    /** ISO timestamp string or null when unknown. */
    last_active: string | null;
}

export interface AiHistoryMessage {
    /** ai_history primary-key row id (a real JS number). */
    id: number;
    local_id: number;
    role: 'user' | 'model';
    content: string;
    /** Discord snowflakes as strings, or null. */
...
/**
 * One undoable, ACK-CONFIRMED mutation. An 'edit' entry is undone by
 * re-sending edit_ai_history_message with the pre-edit content (no new
 * backend op needed); a 'delete' entry is undone by sending the new
 * restore_ai_history_message op with the stored message verbatim.
 */
type UndoEntry =
    | { kind: 'edit'; channelId: string; id: number; prevContent: string }
    | { kind: 'delete'; channelId: string; message: AiHistoryMessage };

export interface HistoryManagerCallbacks {
    /**
     * Emit a WS frame via the shared dashboard socket (ChatManager.send).
     * Returns false (and toasts) when disconnected — callers must roll back
     * any optimistic state on false.
     */
    send: (data: unknown) => boolean;
    /**
     * True when the shared socket is OPEN (ChatManager.connected). Checked
     * BEFORE send() so requests issued while connecting/down queue silently
     * instead of popping ws-client's spurious "Not connected" error toast.
     */
    isConnected: () => boolean;
    /**
     * Kick off a (re)connect of the shared socket (ChatManager.connect).
     * Used by the Refresh button so a click while the socket is down starts
     * a reconnect instead of silently queueing — a dead-looking button.
     */
    connect?: () => void;
    /** Toast injection point for tests; defaults to shared.showToast. */
    notify?: typeof showToast;
    /** Confirm-dialog injection point for tests; defaults to shared.showConfirmDialog. */
    confirmDialog?: typeof showConfirmDialog;
}
...
export class HistoryManager {
    ...
    private pendingChannelLoadId: string | null = null;
    ...
    private pendingChannelsList: boolean = false;
    ...
    private pendingChannelLoad: { channelId: string; limit: number } | null = null;
    /**
    ...
    private lastLoadLimit: number = DEFAULT_LOAD_LIMIT;
    ...
    private refreshFeedbackPending: boolean = false;
    /**
     ...
     */
    private pendingUndoCandidate:
        | { kind: 'edit'; channelId: string; id: number; prevContent: string }
        | { kind: 'delete'; channelId: string; id: number; message: AiHistoryMessage }
        | null = null;
    /**
     ...
     */
    private pendingUndo: { entry: UndoEntry } | null = null;

    private readonly notify: typeof showToast;
    private readonly confirmDialog: typeof showConfirmDialog;
    ...
    private channelFilterDebounce: number | null = null;
    /**
    ...
    private chromeBuilt: boolean = false;

    constructor(private readonly callbacks: HistoryManagerCallbacks) {
    ...
    private buildChannelFilter(): void {
        if (document.getElementById('ai-history-channel-filter')) return;
        ...
        input.addEventListener('input', () => {
            ...
            if (this.channelFilterDebounce !== null) clearTimeout(this.channelFilterDebounce);
    ...
    private buildFindBar(): void {
        ...
        const input = bar.querySelector('#ai-history-search-input') as HTMLInputElement;
    ...
    openFind(): void {
        ...
        const input = document.getElementById('ai-history-search-input') as HTMLInputElement | null;
        if (!bar || !input) return;
        ...
        if (input.value) this.performFind(input.value);
    }
    ...
    private performFind(query: string): void {
        this.clearFindHighlights();
        this.findMatches = [];
        this.findIdx = -1;
        ...
        if (container && query) {
            ...
            if (this.findMatches.length > 0) this.focusFind(0);
        }
        if (countEl) {
    ...
    private focusFind(idx: number): void {
        if (this.findMatches.length === 0) return;
        ...
        this.findIdx = idx;
        const countEl = document.getElementById('ai-history-search-count');
        if (countEl) countEl.textContent = `${idx + 1} / ${this.findMatches.length}`;
    }
    ...
    private unstickMutation(code?: string): void {
        ...
        if (code !== undefined) this.pendingUndoCandidate = null;
        ...
        if (failed !== null && code !== undefined && PERMANENT_HISTORY_ERROR_CODES.has(code)) {
            ...
            if (i !== -1) this.undoStack.splice(i, 1);
        ...
        const container = document.getElementById('ai-history-messages');
        const saveBtn = container?.querySelector('.edit-save-btn') as HTMLButtonElement | null;
        ...
        switch (data.type) {
            case 'ai_channels_list':
                this.channels = Array.isArray(data.channels)
                    ? (data.channels as AiHistoryChannel[])
                    : [];
                this.renderChannelList();
                this.updateHeader();
            ...
            case 'ai_history_loaded': {
                ...
                this.messages = Array.isArray(data.messages)
                    ? (data.messages as AiHistoryMessage[])
                        .map(m => ({
                            ...m,
                            id: Number(m.id),
            ...
            case 'ai_history_message_edited': {
                ...
                this.recoverPendingUndo('edited', cid, editedId,
                ...
                if (target) target.content = editedContent;
                this.editingIdx = null;
                ...
                if (!patched) {
                    ...
                    this.renderMessages();
                    if (container) container.scrollTop = savedPos;
                }
            ...
            case 'ai_history_message_deleted': {
                ...
                if (removedIdx >= 0) this.refocusRow(removedIdx);
                this.updateHeader();
                this.renderLoadAll();
            ...
            case 'ai_history_message_restored': {
                ...
                if (restoredMsg && !this.messages.some(m => m.id === restoredMsg.id)) {
                    ...
                    if (insertAt === -1) insertAt = this.messages.length;
     ...
     */
    private notifyMutationOutcome(data: Record<string, unknown>, op: 'edit' | 'delete' | 'restore'): void {
                ...
                if (data.live_session_patched === false) {
                    this.notify(op === 'edit'
                        ...
                        : op === 'delete'
                            ...
                            : 'Restored in DB; live session not loaded',
    ...
    startEdit(idx: number): void {
        ...
        if (!msg || this.editInFlight || this.pendingChannelLoadId !== null) return;
        const container = document.getElementById('ai-history-messages');
        if (!container) return;
        ...
        const actionsEl = row?.querySelector('.history-msg-actions') as HTMLElement | null;
        if (!contentEl) return;
        ...
        const textarea = contentEl.querySelector('.edit-textarea') as HTMLTextAreaElement | null;
        if (textarea) {
            textarea.focus();
        ...
        textarea?.addEventListener('keydown', (e) => {
            if ((e as KeyboardEvent).key === 'Escape') this.cancelEdit();
        });
    }

    saveEdit(idx: number): void {
        const msg = this.messages[idx];
        if (!msg || this.editInFlight) return;
        const container = document.getElementById('ai-history-messages');
        const row = container?.querySelector(`.history-msg[data-idx="${idx}"]`);
        const textarea = row?.querySelector('.edit-textarea') as HTMLTextAreaElement | null;
        const saveBtn = row?.querySelector('.edit-save-btn') as HTMLButtonElement | null;
        if (!textarea) return;
    ...
    cancelEdit(): void {
        ...
        if (idx !== null) this.refocusRow(idx);
     ...
     */
    private refocusRow(idx: number): void {
        const container = document.getElementById('ai-history-messages');
        if (!container) return;
        ...
        const rows = container.querySelectorAll<HTMLElement>('.history-msg .history-edit-btn');
        const target = container.querySelector<HTMLElement>(
            `.history-msg[data-idx="${idx}"] .history-edit-btn`)
            ?? rows[rows.length - 1]
            ?? container;
    ...
    async requestDelete(idx: number): Promise<void> {
        const msg = this.messages[idx];
        ...
        if (this.editInFlight || this.pendingChannelLoadId !== null) return;
        if (this.editingIdx !== null) {
        ...
        if (this.messages[idx] !== msg) return;

        const container = document.getElementById('ai-history-messages');
        const row = container?.querySelector(`.history-msg[data-idx="${idx}"]`);
        const deleteBtn = row?.querySelector('.history-delete-btn') as HTMLButtonElement | null;
     ...
     */
    private recoverPendingUndo(
        ackType: 'edited' | 'restored',
        channelId: string,
        id: number,
        content?: string,
    ): void {
     ...
     */
    private settleUndoOnAck(
        ackType: 'edited' | 'deleted' | 'restored',
        channelId: string,
        id: number,
    ): void {
        const pending = this.pendingUndo;
    ...
    private renderUndo(): void {
        const btn = document.getElementById('ai-history-undo') as HTMLButtonElement | null;
        if (!btn) return;
     ...
     */
    private setOverflowNote(note: HTMLElement | null, html: string): void {
        if (!note) return;
    ...
    private renderChannelList(): void {
        ...
        const matched = filter
            ? this.channels.filter(ch =>
                (ch.name || `Channel ${ch.channel_id}`).toLowerCase().includes(filter))
            : this.channels;
        ...
        if (hadFocus) {
            (container.querySelector('.history-channel-item[tabindex="0"]') as HTMLElement | null)
                ?.focus();
        }
        ...
        const clickHandler: EventListener = (e) => {
            const target = (e.target as HTMLElement).closest('.history-channel-item[data-channel-id]') as HTMLElement | null;
            if (!target) return;
            const id = target.dataset.channelId;
            if (id) this.openChannel(id);
        };
        ...
        const keyHandler: EventListener = (e) => {
            ...
            const row = (ev.target as HTMLElement).closest('.history-channel-item[data-channel-id]') as HTMLElement | null;
            if (!row) return;
     ...
     */
    private roveChannelFocus(
        container: HTMLElement,
        current: HTMLElement,
        key: string,
    ): void {
        const rows = Array.from(
            container.querySelectorAll<HTMLElement>('.history-channel-item[data-channel-id]'),
        );
     ...
     */
    private showOlderMessages(): void {
        const container = document.getElementById('ai-history-messages');
    ...
    private messageRowHtml(m: AiHistoryMessage, idx: number): string {
    ...
    private renderMessages(): void {
        ...
        if (note && !note.dataset.historyBound) {
            ...
            note.addEventListener('click', (e) => {
                if (!(e.target as HTMLElement).closest('#ai-history-show-older')) return;
    ...
    private bindMessageHandlers(container: HTMLElement): void {
        const slot = container as unknown as Record<string, EventListener | undefined>;
        ...
        const handler: EventListener = (e) => {
            ...
            const editBtn = target.closest('.history-edit-btn') as HTMLElement | null;
            if (editBtn) {
                const idx = Number(editBtn.dataset.idx);
                if (Number.isInteger(idx)) this.startEdit(idx);
                return;
            }
            const deleteBtn = target.closest('.history-delete-btn') as HTMLElement | null;
            if (deleteBtn) {
                const idx = Number(deleteBtn.dataset.idx);
                if (Number.isInteger(idx)) void this.requestDelete(idx);
            }
        };
    ...
    private patchRowContent(container: HTMLElement, id: number, content: string): boolean {
        const row = container.querySelector(`.history-msg[data-id="${id}"]`) as HTMLElement | null;
        if (!row) return false;
        const contentEl = row.querySelector('.history-msg-content');
     ...
     */
    private removeRowInPlace(container: HTMLElement, id: number): boolean {
        const row = container.querySelector(`.history-msg[data-id="${id}"]`) as HTMLElement | null;
        if (!row) return false;
        ...
        if (this.messages.length > MESSAGE_RENDER_CAP) return false;
        if (this.messages.length <= 1) return false;
        const removedIdx = Number(row.dataset.idx);
        row.remove();
        if (Number.isInteger(removedIdx)) this.reindexFrom(container, removedIdx, -1);
        return true;
    }
     ...
     */
    private insertRowInPlace(container: HTMLElement, msg: AiHistoryMessage, insertAt: number): boolean {
        ...
        if (this.messages.length > MESSAGE_RENDER_CAP) return false;
        if (insertAt < start) return false;
        ...
        const newRow = tmp.firstElementChild as HTMLElement | null;
        ...
        const anchor = container.querySelector(
            `.history-msg[data-idx="${insertAt + 1}"]`,
        ) as HTMLElement | null;
        if (anchor) {
    ...
    private reindexFrom(container: HTMLElement, fromIdx: number, delta: number): void {
        container.querySelectorAll<HTMLElement>('.history-msg[data-idx]').forEach(row => {
            const cur = Number(row.dataset.idx);
            if (!Number.isInteger(cur) || cur < fromIdx) return;
            const next = String(cur + delta);
            row.dataset.idx = next;
            row.querySelectorAll<HTMLElement>('[data-idx]').forEach(btn => {
 ...
 */
function wrapHistoryMatches(root: HTMLElement, query: string): HTMLElement[] {
    if (!query) return [];
    const MAX_HITS = 1000;
    ...
    while (n) {
        ...
        if (candidates.length >= MAX_HITS) break;
        n = walker.nextNode();
    }
