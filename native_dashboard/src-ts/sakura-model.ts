...
/** One petal's state for a single frame. Positions are CSS pixels, top-left origin. */
export interface PetalDraw {
    ...
    tumble: number;
    /** per-petal form: blade curl, longitudinal twist, lengthwise arc */
    cup: number;
    twist: number;
    bend: number;
    /** linear RGB in 0..1 */
...
class PetalLayer {
    ...
    private uPersp: WebGLUniformLocation | null;
    ...
    constructor(zIndex: number) {
        ...
        if (!gl) {
            ...
            this.instanceBuf = null as unknown as WebGLBuffer;
            this.indexCount = 0;
            this.uViewport = null;
            this.uPersp = null;
            this.uLight = null;
    ...
    draw(petals: PetalDraw[], cssW: number, cssH: number, light: [number, number, number]): void {
        if (!this.ok) return;
        const gl = this.gl;
...
export class SakuraRenderer {
    ...
    render(petals: PetalDraw[], light: [number, number, number]): void {
        if (!this.ok) return;
