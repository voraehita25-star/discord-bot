...
export interface PetalDraw {
     ...
     *  ~-0.44 = turn about the base (blossom lobes), slightly past the centre
     *  so the five claws overlap instead of meeting on it */
    pivot: number;
    /** taper exponent, ~1.15 (long narrow neck) … ~2.1 (broad shoulders) */
    neck: number;
    /** tip roundness, ~0.12 (nearly square) … ~0.52 (full arc) */
    dome: number;
    /** notch depth, as a fraction of the blade's length. ~0.04 (the barely-cleft
     *  yaezakura) … ~0.25 (deeply cleft). Read together with `dome`: the two
     *  cut the same end, so a deep notch under a wide dome is a shallow dip
     *  between two low shoulders, not a split. */
    notch: number;
...
export class SakuraRenderer {
    ...
    private buffersSized = false;
    private restoreFired = false;
