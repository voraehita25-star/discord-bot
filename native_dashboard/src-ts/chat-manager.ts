...
const HISTORY_ERROR_SCOPES: ReadonlySet<string> = new Set([
    'ai_history',
    'list_ai_channels',
    'load_ai_history',
...
const CHAT_ERROR_SCOPES: ReadonlySet<string> = new Set([
    'message',
    'ai_edit_message',
    'edit_message',
    'delete_message',
...
function chatFileCount(value: unknown): number {
...
export class ChatManager {
    ...
    streamingConversationId: string | null = null;
    ...
    private streamErrored: boolean = false;
    ...
    private streamPartialChunks: string[] = [];
    private streamPartialThinkingChunks: string[] = [];
    private streamThinkingActive: boolean = false;
    ...
    private followStreamRafId: number | null = null;
    ...
    private pendingConversationLoadId: string | null = null;
    ...
    readonly wsClient: WebSocketClient = new WebSocketClient({
        ...
        onDisconnect: () => {
            ...
            if (this.isStreaming) this.forceStreamTeardown();
        },
    });
    ...
    forceStreamTeardown(): void {
        ...
        if (wasEditStreaming) {
            ...
            document.querySelectorAll('.chat-message.streaming').forEach(el => {
                ...
                const actions = el.querySelector('.message-actions') as HTMLElement | null;
    ...
    send(data: unknown): boolean { return this.wsClient.send(data); }
    ...
    handleMessage(data: Record<string, unknown>): void {
        switch (data.type) {
            ...
            case 'conversation_loaded': {
                ...
                const incoming = this.enrichConversation(data.conversation as ChatConversation);
            ...
            case 'message_deleted':
                {
                    ...
                    if (data.cli_session_diverged === true) {
                        showToast(
                            'Deleted — but the AI session may replay this next turn. Reload the conversation to force a fresh session.',
                            { type: 'warning', duration: 8000 },
                        );
                    }
                }
            ...
            case 'conversation_untagged':
                if (this.currentConversation
                    ...
                    this.currentConversation.tags = data.tags as string[];
            ...
            case 'provider_updated': {
                ...
                const aiProvider = data.ai_provider as string | undefined;
                if (
                    conversationId &&
                    aiProvider &&
                ...
                window.dispatchEvent(
                    new CustomEvent('ai-provider-updated', {
    ...
    loadConversation(id: string): void {
        ...
        if (this.draftSaveTimer !== null) {
            ...
            if (this.currentConversation) {
                ...
                if (el) this.saveDraft(this.currentConversation.id, el.value);
            }
        }
    ...
    private restoreStreamingBubble(): void {
        ...
        if (this.currentThinking) {
            ...
            if (liveThinking) {
                ...
                if (thinkingContent) thinkingContent.appendChild(document.createTextNode(liveThinking));
            }
        }
        this.scrollToBottom();
    }
        ...
        if (streamingMsg) {
            ...
            if (wrapper && !wrapper.querySelector('.message-actions')) {
                ...
                actionsDiv.querySelector('.delete-message-btn')?.addEventListener('click', async () => {
                    ...
                    if (!confirmed) return;
                    const idx = this.messages.indexOf(newMessage);
                    if (idx >= 0) this.deleteMessage(idx);
                });
            }
        }
    ...
    appendEditStreamChunk(text: string): void {
        ...
        if (this.editStreamRafId !== null) return;
        ...
        container.querySelectorAll('.message-image[data-img-idx]').forEach(img => {
            img.addEventListener('click', () => {
                ...
                if (src) {
                    ...
                    if (newWindow) {
                        try { newWindow.opener = null; } catch { /* cross-origin guard */ }
    ...
    private syncComposerMode(): void {
        ...
        const stopBtn = document.getElementById('btn-stop-generating') as HTMLButtonElement | null;
    ...
    followStream(): void {
        ...
        if (this.followStreamRafId !== null) return;
    ...
    private attachModalDismiss(close: () => void): void {
        this.detachModalDismiss();
        this.modalPreviousFocus = document.activeElement as HTMLElement | null;
    ...
    private detachModalDismiss(): void {
    ...
    private focusIntoModal(modal: HTMLElement, preferred?: HTMLElement | null): void {
        const target = preferred ?? modal.querySelector<HTMLElement>('.modal-close');
        target?.focus();
    }
    ...
    openChatFilesModal(): void {
        ...
        this.attachModalDismiss(() => {
            ...
            if (editing) this.closeChatFileEditor();   // it handles its own focus handoff
            else this.closeChatFilesModal();
        });
    ...
    closeChatFileEditor(): void {
        ...
        if (wasFocusInEditor) {
            (document.getElementById('chat-files-close') as HTMLElement | null)?.focus();
        }
    }
    ...
    saveChatFileEditor(): void {
        ...
        if (this.editingDocId == null || !this.currentConversation) return;
        const nameInput = document.getElementById('chat-files-edit-name') as HTMLInputElement | null;
        const textInput = document.getElementById('chat-files-edit-text') as HTMLTextAreaElement | null;
    ...
    showNewChatModal(): void {
        ...
        if (modal) {
            ...
            this.focusIntoModal(
                modal,
                modal.querySelector<HTMLElement>('.role-card.selected'),
            );
        }
    }
    ...
    init(): void {
        ...
        document.getElementById('chat-ai-provider')?.addEventListener('change', (e) => {
            ...
            if (!this.availableProviders.includes(selected)) {
                ...
                (e.target as HTMLSelectElement).value = this.aiProvider;
                return;
            }
