...
export class ChatSearch {
    ...
    private inputDebounce: number | null = null;
    ...
    setup(): void {
        ...
        input.addEventListener('input', () => {
            if (this.inputDebounce !== null) clearTimeout(this.inputDebounce);
     ...
     *  the N shown. */
    private formatCount(current: number): string {
...
/** Result of a wrapMatches pass: the wrapped <mark> nodes plus whether the
 *  pass stopped early at one of the caps (so callers can show a "+N" hint). */
...
function wrapMatches(root: HTMLElement, query: string): WrapResult {
    if (!query) return { marks: [], truncated: false };
    ...
    while (n) {
        ...
        if (candidates.length >= MAX_CANDIDATE_NODES) {
            ...
            if (walker.nextNode() !== null) truncated = true;
            break;
        }
        n = walker.nextNode();
    }
    ...
    outer: for (let ci = 0; ci < candidates.length; ci++) {
        ...
        for (const m of text.matchAll(re)) {
            ...
            if (hits.length >= MAX_HITS) {
                ...
                if (tail || ci < candidates.length - 1) truncated = true;
                break outer;
            }
        }
