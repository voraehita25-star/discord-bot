 ...
 */
export function stripThinkTagsStreaming(content: string): string {
    return content
...
interface ListItem {
    ...
    indent: number;
    ordered: boolean;
    /** The number the author actually wrote (`3.` → 3); 1 for bullets. */
    number: number;
...
function buildListHtml(items: ListItem[]): string {
    ...
    const open = (it: ListItem): string =>
        it.ordered ? `<ol${it.number === 1 ? '' : ` start="${it.number}"`}>` : '<ul>';
    const close = (ordered: boolean): string => (ordered ? '</ol>' : '</ul>');
