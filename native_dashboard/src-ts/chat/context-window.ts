...
export class ContextWindowIndicator {
    private cache: Map<string, TokenUsage> = new Map();
    ...
    load(): void {
        try {
            ...
            for (const [k, v] of Object.entries(parsed as Record<string, unknown>)) {
                ...
                const u = v as Partial<TokenUsage>;
                // Reject NaN/non-finite numbers — they would propagate into
                // ``pct`` math and render "NaN%" in the UI forever until
                // localStorage is cleared.
                if (typeof u.input_tokens !== 'number' || !Number.isFinite(u.input_tokens)) continue;
                if (typeof u.output_tokens !== 'number' || !Number.isFinite(u.output_tokens)) continue;
                if (typeof u.context_window !== 'number' || !Number.isFinite(u.context_window)) continue;
                ...
                if (u.context_window <= 0) continue;
    ...
    private paint(usage: TokenUsage): void {
        ...
        else if (pct >= 50) fill.classList.add('usage-moderate');
     ...
     */
    addPendingDocumentChars(conversationId: string | null, chars: number): void {
        if (!conversationId || !Number.isFinite(chars) || chars <= 0) return;
        const tokens = Math.ceil(chars / 4);
        this.pendingDocTokens.set(
            conversationId,
            (this.pendingDocTokens.get(conversationId) ?? 0) + tokens,
        );
        const cached = this.cache.get(conversationId);
        if (cached) this.paint(this.withPending(conversationId, cached));
    }

    /** Overlay a conversation's pending-document estimate onto a usage reading. */
    private withPending(conversationId: string, usage: TokenUsage): TokenUsage {
        const pending = this.pendingDocTokens.get(conversationId) ?? 0;
        if (pending <= 0) return usage;
        return {
            ...usage,
    ...
    setDocumentFootprint(
        ...
        if (newTokens < prevTokens) {
            ...
            const pending = Math.max(
                0,
                (this.pendingDocTokens.get(conversationId) ?? 0) - (prevTokens - newTokens),
            );
            if (pending > 0) this.pendingDocTokens.set(conversationId, pending);
            else this.pendingDocTokens.delete(conversationId);
        }
    ...
    private save(): void {
        ...
        const toSave = entries.length > MAX_CACHE_SIZE
            ...
            : entries;
        const obj: Record<string, TokenUsage> = {};
