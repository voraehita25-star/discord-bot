...
const PRISM_LANGS: ReadonlySet<string> = new Set([
...
const PRISM_ALIASES: Readonly<Record<string, string>> = {
    js: 'javascript',
    ts: 'typescript',
    py: 'python',
    rb: 'ruby',
 ...
 */
const PRISM_DEPS: Readonly<Record<string, string[]>> = {
    c: ['clike'],
    cpp: ['c'],
    javascript: ['clike'],
...
function getPrism(): PrismGlobal | undefined {
    return (window as unknown as { Prism?: PrismGlobal }).Prism;
}

/** Normalize alias → canonical Prism id (js → javascript, py → python, …). */
export function canonicalPrismLang(lang: string): string {
...
export async function loadPrismLanguage(lang: string): Promise<void> {
    ...
    if (!PRISM_LANGS.has(canon)) return;
...
export async function highlightCodeBlocks(root: HTMLElement): Promise<void> {
    ...
    for (const code of Array.from(codes)) {
        ...
        if (supported) await loadPrismLanguage(lang);
        if (prism.languages[canon]) {
            try {
