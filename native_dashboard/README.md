...
| 🤖 **Headless E2E** | 161 Playwright tests across 13 spec files in `tests-e2e/` — UI smoke, user-flow interactions, axe-core a11y audit, pixel-sampled contrast (low-emphasis text + non-text/1.4.11), populated-state a11y + layout, visual-regression snapshots, H5 import-map IPC, H7 strict-CSP render, deep UI inspection, upgrade-audit regression guards (the page-sweep specs cover the History page too). Runs in CI on Chromium with python http.server + mocked Tauri IPC. A real (non-mock) Tauri Rust-IPC round-trip is covered by `scripts/dev/validate_ipc.py` (tauri-driver/WebView2). |
...
npm test                       # Run all 624 vitest tests
...
npm run test:e2e               # Run all 161 Playwright tests (smoke + interactions + a11y + contrast + visual + h5/h7 + inspection + upgrade-guards)
