...
class TestTimestampParsing:
    """Tests for _parse_history_timestamp and _normalize_history_timestamp."""

    def test_parse_iso_timestamp(self):
        """Parse standard ISO 8601 timestamp."""
        from cogs.ai_core.storage import _parse_history_timestamp

        dt = _parse_history_timestamp("2026-01-15T10:30:00+00:00")
    ...
    def test_parse_utc_z_suffix(self):
        """Parse timestamp ending with Z (UTC)."""
        from cogs.ai_core.storage import _parse_history_timestamp

        dt = _parse_history_timestamp("2026-04-09T12:00:00Z")
        assert dt is not None
        assert dt.hour == 12

    def test_parse_naive_timestamp_gets_utc(self):
        """Parse naive timestamp — should get UTC timezone."""
        from cogs.ai_core.storage import _parse_history_timestamp

        dt = _parse_history_timestamp("2026-04-09T12:00:00")
        assert dt is not None
    ...
    def test_parse_invalid_returns_none(self):
        ...
        assert _parse_history_timestamp("not-a-date") is None
        assert _parse_history_timestamp("") is None
        assert _parse_history_timestamp(None) is None
        assert _parse_history_timestamp(12345) is None

    def test_normalize_produces_iso(self):
        """Normalize produces consistent ISO output."""
        from cogs.ai_core.storage import _normalize_history_timestamp

        result = _normalize_history_timestamp("2026-04-09T12:00:00Z")
        assert result is not None
        assert "+00:00" in result

    def test_normalize_invalid_returns_none(self):
        """Normalize returns None for invalid input."""
        from cogs.ai_core.storage import _normalize_history_timestamp

        assert _normalize_history_timestamp("garbage") is None
        assert _normalize_history_timestamp(None) is None
