...
class TestURLFetcherClientFetchFallback:
    ...
    async def test_fetch_fallback_success_html(self, _mock_ssrf):
        ...
        # _fetch_fallback drains the body via a capped ``resp.content.read()``
        # loop and then decodes the bytes. Hand it real bytes followed by b""
        # (EOF) so the loop terminates on the first window; without the EOF the
        # loop would keep re-reading the same bytes up to the 5 MB cap. Real
        # bytes (not an auto-created AsyncMock) so the synchronous ``.decode()``
        # runs on bytes instead of a never-awaited coroutine.
        mock_response.content.read = AsyncMock(side_effect=[html_content.encode(), b""])
        mock_response.__aenter__ = AsyncMock(return_value=mock_response)
        mock_response.__aexit__ = AsyncMock()
