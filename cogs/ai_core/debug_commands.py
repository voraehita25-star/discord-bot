...
class AIDebug(commands.Cog):
    ...
    async def ai_debug(self, ctx: Context) -> None:
        ...
        embed.add_field(
            name="📝 Session",
            value=f"```\n{session_info}\nTokens: ~{token_count:,}\nThinking: {'✅' if thinking_enabled else '❌'}```",
            inline=True,
        )
        ...
        try:
            ...
            rag_info = (
                ...
                f"Index: {'✅ ' + str(stats['index_size']) + ' vectors' if stats['index_built'] else '❌ Not built'}\n"
    ...
    async def ai_trace(self, ctx: Context) -> None:
        ...
        if last_trace:
            ...
            embed.add_field(
                name="🔢 Tokens",
                value=f"```\nInput: {last_trace.get('input_tokens', 'N/A')}\nOutput: {last_trace.get('output_tokens', 'N/A')}```",
                inline=True,
            )
