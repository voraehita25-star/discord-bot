...
MAX_WEBHOOK_CACHE_SIZE = 500  # Max channels to cache - prevents unbounded growth
...
async def _cleanup_expired_webhook_cache() -> None:
    ...
    while True:
        try:
            ...
            logger.exception("Error in webhook cache cleanup")
...
def start_webhook_cache_cleanup(bot) -> None:
    ...
    try:
        ...
        logger.warning("🧹 Cannot start webhook cleanup: event loop not ready")
        return
...
async def stop_webhook_cache_cleanup() -> None:
    ...
    if task_to_cancel:
        ...
        logger.info("🧹 Stopped webhook cache cleanup task")
...
def set_cached_webhook(channel_id: int, webhook_name: str, webhook: discord.Webhook) -> None:
    ...
    with _webhook_cache_lock:
        if channel_id not in _webhook_cache:
            ...
            if len(_webhook_cache) >= MAX_WEBHOOK_CACHE_SIZE and _webhook_cache_time:
                oldest_id = min(_webhook_cache_time, key=lambda cid: _webhook_cache_time[cid])
                _webhook_cache.pop(oldest_id, None)
                _webhook_cache_time.pop(oldest_id, None)
...
def invalidate_webhook_cache_on_channel_delete(channel_id: int) -> None:
    ...
    logger.debug("🧹 Invalidated webhook cache for deleted channel %s", channel_id)
