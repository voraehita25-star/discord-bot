...
# for the deepest reasoning (Opus-tier only). Only forwarded to the API when
# valid; unknown values fall back to None so a typo cannot break requests.
_CLAUDE_EFFORT_ALLOWED = frozenset({"low", "medium", "high", "xhigh", "max"})
_raw_claude_effort = os.getenv("CLAUDE_EFFORT", "max").strip().lower()
