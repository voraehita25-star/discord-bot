...
# (deep Opus-tier reasoning, one tier below ``max``). Only forwarded to the API
# when valid; unknown values fall back to None so a typo cannot break requests.
_CLAUDE_EFFORT_ALLOWED = frozenset({"low", "medium", "high", "xhigh", "max"})
_raw_claude_effort = os.getenv("CLAUDE_EFFORT", "xhigh").strip().lower()
CLAUDE_EFFORT: str | None = (
    _raw_claude_effort if _raw_claude_effort in _CLAUDE_EFFORT_ALLOWED else None
)
