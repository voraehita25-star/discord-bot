...
class ContextBuilder:
    ...
    async def build_context(
        ...
        if tasks:
            ...
            for name, result in zip(task_names, results, strict=False):
                # Never swallow CancelledError — propagate so shutdown/timeout logic works.
                if isinstance(result, asyncio.CancelledError):
                    raise result
                if isinstance(result, BaseException):
                    logger.error("Context build error for %s: %s", name, result)
                    continue
    ...
    async def _get_entity_memory(self, channel_id: int, user_id: int, message: str) -> str:
        ...
        try:
            ...
            logger.exception("Entity memory error")
            return ""
    ...
    async def _get_state_tracker(self, channel_id: int) -> str:
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.debug("State tracker retrieved in %.2fs", elapsed)

            return state or ""

        except Exception:
            logger.exception("State tracker error")
            return ""
    ...
    async def _get_avatar_context(self, channel_id: int, guild: Any) -> dict[str, Any] | None:
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.debug("Avatar retrieved in %.2fs", elapsed)
            ...
            logger.exception("Avatar context error")
            return None
    ...
    async def fetch_url_content(self, urls: list[str], max_content_length: int = 2000) -> str:
        ...
        try:
            ...
            logger.exception("URL content fetch error")
            return ""
