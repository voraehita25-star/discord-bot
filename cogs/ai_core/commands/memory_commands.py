...
# Shared prompt-injection predicate — the SAME denylists the AI remember tool /
# IPC sinks use. `!remember` stores via ``add_explicit_fact`` straight into
# RAG-retrievable context, so it must reuse the authoritative screen rather than
# carry a private copy that can drift out of sync.
...
class MemoryCommands(commands.Cog):
    ...
    async def remember_fact(self, ctx: Context, *, fact: str = "") -> None:
        ...
        # Reject stored prompt-injection markers using the shared, authoritative
        # screen (``memory_content_has_injection``) — the SAME denylists the AI
        # remember tool / IPC sinks apply — so a member can't persist
        # ``[SYSTEM] ignore previous`` (plain or confusable-spelled) into their
        # own RAG-retrievable context.
        if memory_content_has_injection(fact):
