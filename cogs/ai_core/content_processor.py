...
Handles image, attachment, and avatar processing for AI interactions.
...
async def prepare_user_avatar(
    ...
    Note:
        ...
        PIL Images used in Gemini API content will be automatically handled
        when converted to inline_data format via pil_to_inline_data().
    """
...
TEXT_EXTENSIONS = (
    ...
    ".sh", ".bat", ".sql", ".java", ".c", ".cpp", ".h", ".rs",
    ".go", ".rb", ".php", ".lua", ".r", ".vue", ".jsx", ".tsx",
)
...
async def process_attachments(
    ...
    Args:
        ...
        user_name: Display name of user for logging.
        is_animated_gif_func: Function to check if GIF is animated.
        convert_gif_to_video_func: Function to convert GIF to video.
            ...
            try:
                ...
                if len(content) > chunk_size:
                    ...
                    logging.info(
                        ...
                        attachment.filename, user_name, len(content), total_chunks,
                    )
                else:
                    text_parts.append(
                    ...
                    logging.info(
                        ...
                        attachment.filename, user_name, len(content),
                    )
        ...
        if attachment.content_type and attachment.content_type.startswith("image/"):
            try:
                ...
                if (
                    ...
                    and IMAGEIO_AVAILABLE
                    and is_animated_gif_func
                    and convert_gif_to_video_func
                ):
                    if is_animated_gif_func(image_data):
                        video_bytes = convert_gif_to_video_func(image_data)
                        if video_bytes:
...
def is_animated_gif(image_data: bytes) -> bool:
    ...
    Args:
        image_data: Raw bytes of GIF image.
...
def convert_gif_to_video(gif_data: bytes) -> bytes | None:
    ...
    Args:
        gif_data: Raw bytes of animated GIF.

    Returns:
        MP4 video bytes if conversion successful, None otherwise.
    """
...
def load_character_image(
    ...
    Args:
        message: User's message to search for character names.
        guild_id: Guild ID to look up character map.

    Returns:
        Tuple of (character_name, PIL Image) if found, None otherwise.

    Note:
        Uses cached image bytes for better performance.
    """
