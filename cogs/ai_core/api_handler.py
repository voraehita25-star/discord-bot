...
- SEARCH = Need up-to-date info from web, wiki, or external source
  Examples: game stats, Identity details, rarity info, news, wiki data, fact-checking, specific numbers
- NO_SEARCH = Can answer from general knowledge, roleplay, chat, creative writing, opinions

IMPORTANT: For questions about Limbus Company, Project Moon, Identities, E.G.O, character stats, rarity levels, skill info - reply "SEARCH" because these need accurate wiki data.
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            try:
                ...
                logging.error("⏱️ Gemini API timeout after %.0fs (attempt %d)", api_timeout, attempt + 1)
                if CIRCUIT_BREAKER_AVAILABLE and gemini_circuit:
                    gemini_circuit.record_failure()
