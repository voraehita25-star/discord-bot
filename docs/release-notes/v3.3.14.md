...
- **2,968 passed**, 0 failed, 2 warnings (harmless aiosqlite DeprecationWarning)

## 🔍 Comprehensive Audit (March 25, 2026)

### Security Fixes

- **Fix: Path traversal in RAG engine `save()`/`load()`** — No path validation allowed `..` components to escape target directory. Added `Component::ParentDir` check. (`rust_extensions/rag_engine/src/lib.rs`)
- **Fix: Timestamp comparison without ISO validation** — History diff used string comparison on timestamps without format validation, risking silent data corruption. Added `_ISO_RE` regex validation. (`cogs/ai_core/storage.py`)

### Reliability Fixes

- **Fix: Task leak on cog hot-reload** — Old background tasks (`cleanup_task`, `_pending_request_cleanup_task`, `_cache_cleanup_task`) were not cancelled before creating new ones. (`cogs/ai_core/ai_cog.py`)
- **Fix: Bot instance memory leak on restart** — Old bot instance was not explicitly closed before creating a new one in the restart loop. (`bot.py`)
- **Fix: Missing database indexes** — Added indexes for `knowledge_entries` (domain, category, topic), `error_logs` (type, created_at) to avoid full table scans. (`utils/database/database.py`)
- **Fix: audit_log index optimization** — Changed `idx_audit_log_created` to DESC order and added composite `idx_audit_log_guild_created`. (`utils/database/database.py`)

## Files Changed

- **Modified:** 16 source files, 1 test file, 6 docs
- **Security:** 8 fixes | **Reliability:** 9 fixes | **Tests:** 2 fixes
