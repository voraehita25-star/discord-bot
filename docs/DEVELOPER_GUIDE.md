...
> **Total Files:** 124 Python test files (5,338 tests) + 21 vitest files (613 frontend tests) + 13 Playwright spec files (159 e2e + a11y + visual regression tests)
...
## 📁 Directory Structure (267 Python Files)
...
│
├── tests/                    # 🧪 Python test suite (5,338 tests in 124 files)
│   ├── __init__.py
...
│   │   ├── chat-manager.test.ts       # ChatManager dispatcher + state (103 tests)
│   │   ├── history-manager.test.ts    # AI History page (114 tests)
...
│   │       └── *.test.ts     # 21 vitest files total (613 tests)
│   ├── tests-e2e/            # Playwright (Chromium) — headless against the static UI (159 tests, incl. the history page)
...
│   │   ├── interactions.spec.ts         # 20 user-flow tests
│   │   ├── a11y.spec.ts                 # 14 axe-core audits
│   │   ├── visual-regression.spec.ts    # 9 baseline screenshots
...
│   │   ├── upgrade-guards.spec.ts       # 8 upgrade-audit regression guards
│   │   └── screenshots.spec.ts          # 11 manual-inspection captures
